...
<section class="orders">
    <div class="container">
        <div class="flex_titles_account">
           @if (isset($status))
                <a href='{{ route('account.burial.filter',1) }}'class="btn_bac_gray<?php if($status==1){echo ' active_label_product';}?>">Оплаченные</a>
                <a href='{{ route('account.burial.filter',0) }}'class="btn_bac_gray<?php if($status==0){echo ' active_label_product';}?>">Ожидают оплаты </a>
                <a href='{{ route('account.burial.favorite') }}'class="btn_bac_gray"><img src='{{ asset('storage/uploads/Star 1 (2).svg') }}'>Избранное </a>
        ...
        @if(isset($orders_products))
            @if(count($orders_products)>0)
                @foreach ($orders_products as $orders_product)
                    <?php $product=Burial::findOrFail($orders_product->product_id);?>
                    <div class="li_order">
                        ...
                        @if($orders_product->status==0)
                            ...
                            <a href='{{ route('account.burial.delete',$orders_product->id) }}'class="delete_cart"><img src="{{asset('storage/uploads/Trash.svg')}}" alt=""> Отменить</a>
