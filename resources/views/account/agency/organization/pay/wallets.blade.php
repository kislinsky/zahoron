    ...
    <div class="ul_wallets">
        @foreach ($wallets as $key=>$wallet)
            <div class="li_wallet">
                ...
                <form class='block_input' action="{{ route('account.agency.organization.wallet.update.balance') }}" method="post">
                    @csrf
                    <input type="hidden" name="wallet_id" value={{ $wallet->id }}>
                    <input type="number" name="count" id="" value=3000>
                    <button class='blue_btn'>Пополнить</button>
                </form>
                <form action="{{ route('account.agency.organization.wallet.delete',$wallet->id) }}" method="post">
                    @csrf
                    @method('DELETE')
