...
@section('title', "Завершенные заявки  по ритуальным услугам")
...
$( ".service_filter .select select" ).on( "change", function() {
    ...
    $.ajax({
            ...
            success: function (result) {
                ...
                let st = strings.join("&")
                window.history.pushState('funeral-service', 'Title', '/{{selectCity()->slug}}/account/agency/aplication/funeral-service/completed?'+st);
            },
            error: function () {
                alert('Ошибка');
                $('.bac_loader').fadeOut()
