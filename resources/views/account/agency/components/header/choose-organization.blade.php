...
<div class="choose_organization_block">
    ...
    @if(user()->organization()!=null)
        <div class="info_selected_organization">
            <div class="text_block open_ul_agency_organizations">{{user()->organization()->title}} {{user()->organization()->adres}}  <img class='img_light_theme' src='{{asset('storage/uploads/Vector 9 (1).svg')}}'>
    ...
    @endif
        ...
        @if(user()->organizations->count()>0)
            @foreach (user()->organizations as $organization)
                <a href='{{route('account.agency.choose.organization',$organization->id)}}' class="li_agency_organization text_block">{{$organization->title}} {{$organization->adres}} <div class="blue_btn">выбрать</div></a>   
            @endforeach
        @else
