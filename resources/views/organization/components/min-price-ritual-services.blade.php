@if($categories_organization!=null && count($categories_organization)>0)
    @foreach ($categories_organization as $category_organization)
        @php 
            $categories_children=childrenCategoryOrganization($organization,$category_organization);
        @endphp
        <div class="block_content_organization_single">
            <h2 class="title_li title_price_organization_ritual_services">Минимальная стоимость {{$category_organization->title}} "{{ $organization->title }}"</h2>
