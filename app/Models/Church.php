...
class Church extends Model
{
    ...
    public function views(){
        return $this->hasMany(View::class, 'entity_id')->where('entity_type', 'church');
    }


    public function timeEndWorkingNow(){
        $day=addHoursAndGetDay($this->time_difference);
        $get_hours=WorkingHoursChurch::where('church_id',$this->id)->where('day',$day)->first();
        if($get_hours!=null){
    ...
    {    
        return WorkingHoursChurch::where('church_id', $this->id)
    ...
    {
        return $this->hasMany(ImageChurch::class);
    }
    ...
    {
        return ReviewChurch::where('church_id', $this->id)->where('status', 1)->count();
    }
    ...
    {    
        $days = WorkingHoursChurch::where('church_id', $this->id)
            ->orderByRaw("FIELD(day, 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday')")
            ->get();
    ...
    {
        return $this->hasMany(WorkingHoursChurch::class);
    }
}