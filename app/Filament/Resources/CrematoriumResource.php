...
use App\Filament\Resources\CrematoriumResource\RelationManagers\ImagesRelationManager;
...
class CrematoriumResource extends Resource
{
    protected static ?string $model = Crematorium::class;

    protected static ?string $navigationIcon = 'heroicon-o-rectangle-stack';
    protected static ?string $navigationLabel = 'Крематории'; // Название в меню
            ...
            ->schema([
                ...
                RichEditor::make('content') // Поле для редактирования HTML-контента
                    ...
                    FileUpload::make('img_file')
                        ...
                        ->directory('/uploads_crematoriums') // Директория для хранения файлов
                        ->image()
                        ->maxSize(2048)
                        ->reactive()
    ...
    public static function table(Table $table): Table
    {
        ...
        $cemeteries = $query->with(['city.area.edge', 'city.area', 'city'])
            ->get()
            ->map(function ($crematorium) {
                return [
                    'ID' => $crematorium->id,
                    'Название' => $crematorium->title,
                    'Ширина' => $crematorium->width,
                    ...
                    'Телефон' => $crematorium->phone,
                    'Рейтинг' => $crematorium->rating,
                ];
            });

        // Если данные пустые, возвращаем все записи
        if ($cemeteries->isEmpty()) {
            $cemeteries = Crematorium::query()
    ...
    {
        return [
            'index' => Pages\ListCrematoria::route('/'),
            'create' => Pages\CreateCrematorium::route('/create'),
            'edit' => Pages\EditCrematorium::route('/{record}/edit'),
        ];
    }
}
