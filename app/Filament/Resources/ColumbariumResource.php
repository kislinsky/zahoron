...
use App\Filament\Resources\ColumbariumResource\RelationManagers\ImagesRelationManager;
use App\Filament\Resources\ColumbariumResource\RelationManagers\ViewsRelationManager;
use App\Filament\Resources\ColumbariumResource\RelationManagers\WorkingHoursRelationManager;
use App\Models\Area;
use App\Models\City;
...
class ColumbariumResource extends Resource
{
            ...
            ->schema([
                ...
                RichEditor::make('content') // Поле для редактирования HTML-контента
                    ...
                    FileUpload::make('img_file')
                        ...
                        ->directory('/uploads_columbariums') // Директория для хранения файлов
                        ->image()
                        ->maxSize(2048)
                        ->reactive()
    ...
    public static function table(Table $table): Table
    {
        ...
        $cemeteries = $query->with(['city.area.edge', 'city.area', 'city'])
            ->get()
            ->map(function ($columbarium) {
                return [
                    'ID' => $columbarium->id,
                    'Название' => $columbarium->title,
                    'Ширина' => $columbarium->width,
                    ...
                    'Телефон' => $columbarium->phone,
                    'Рейтинг' => $columbarium->rating,
                ];
            });

        // Если данные пустые, возвращаем все записи
        if ($cemeteries->isEmpty()) {
            $cemeteries = Columbarium::query()
