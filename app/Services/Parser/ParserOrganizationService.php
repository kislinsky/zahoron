...
class ParserOrganizationService
{
    public static function index($request)
    {
        ...
        $importType = $data->input('import_type'); // 'new' или 'update'
        $importWithUser = $data->input('import_with_user', 0);
        $columnsToUpdate = $data->input('columns_to_update', []);

        $importedFiles = [];

        // Параметры фильтрации
        $filterRegion = $data->input('filter_region');
        $filterDistrict = $data->input('filter_district');
        $filterCity = $data->input('filter_city');
        ...
        foreach ($files as $file) {
            ...
            foreach($organizations as $organization) {
                ...
                if (!$two_gis_link && isset($columns['2GIS ссылка'])) {
                    $two_gis_link = $organization[$columns['2GIS ссылка']] ?: null;
                }
                ...
                if (!$existingOrg && $two_gis_link) {
                    $existingOrg = Organization::where('two_gis_link', $two_gis_link)->first();
                }
                ...
                if ($skipByUserCondition){
                    Log::info($orgId . ': пропущен, потому что $skipByUserCondition = true');
                    continue;
                }
                ...
                if ($importType == 'update') {
                    ...
                    if (!$existingOrg){
                        Log::info($orgId . ': пропущен, потому что $existingOrg = false');
                        continue;
                    }
                    ...
                    if (in_array('inn', $columnsToUpdate) && isset($columns['inn'])) {
                        if ($innOrg) {
                            ...
                            if (strlen($cleaned) === 10 || strlen($cleaned) === 12) {
                                ...
                                Log::info($orgId . ': пропущен, потому что strlen($innOrg) = ' . strlen($cleaned));
                            }
                        }
                    }
