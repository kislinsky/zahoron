...
class WorkingHoursCemeterySeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $oldData = DB::connection('zahoron_old')->table('working_hours_cemeteries')->get();

        // Переносим данные в `working_hours_cemeteries`
        foreach ($oldData as $item) {
            // Проверяем существование cemetery_id
            $cemeteryExists = DB::connection('zahoron_old')->table('cemeteries')->where('id', $item->cemetery_id)->exists();

            // Если кладбище существует, добавляем запись
            if ($cemeteryExists) {
                DB::table('working_hours_cemeteries')->insert([
