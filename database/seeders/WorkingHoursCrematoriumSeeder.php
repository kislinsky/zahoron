...
class WorkingHoursCrematoriumSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $oldData = DB::connection('zahoron_old')->table('working_hours_crematoria')->get();

        // Переносим данные в `working_hours_cemeteries`
        foreach ($oldData as $item) {
            // Проверяем существование cemetery_id
            $cemeteryExists = DB::connection('zahoron_old')->table('crematoria')->where('id', $item->crematorium_id)->exists();

            // Если кладбище существует, добавляем запись
            if ($cemeteryExists) {
                DB::table('working_hours_crematoria')->insert([
                    'time_start_work' => $item->time_start_work ?? '00:00',
                    'time_end_work'   => $item->time_end_work ?? '00:00',
                    'holiday'         => $item->holiday ?? 0,
                    'crematorium_id'     => $item->crematorium_id,
